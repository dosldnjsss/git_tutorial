...
function checkWrite(){
	...
	if(document.writeForm.name.value==""){
		...
		document.writeForm.submit();/*submit~action(페이지 이동 + 데이터) - input 데이터를 들고 간다. */
	}
}
...
function execDaumPostcode() {
    new daum.Postcode({
        oncomplete: function(data) {
            ...
            // 커서를 상세주소 필드로 이동한다.
            document.getElementById("addr2").focus();//focus() : 커서가 깜빡인다.
        }
    }).open();
}
...
function checkId(){
	...
	if(id == ""){
		...
		window.open("/memberServlet/CheckIdServlet?id="+id,
